...
#[tauri::command]
pub async fn import_image(app_handle: AppHandle, image_data: &str, file_name: Option<String>) -> Result<String, String> {
...
/// Import an image by copying it from a local filesystem path into the app's
/// assets directory. Returns a relative path like "assets/<filename>" suitable
/// for Markdown insertion.
#[tauri::command]
pub async fn import_image_from_path(app_handle: AppHandle, source_path: &str) -> Result<String, String> {
    ...
    let orig_name = src.file_name().and_then(|n| n.to_str()).unwrap_or("image.png");
