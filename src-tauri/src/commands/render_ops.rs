...
#[tauri::command]
pub async fn render_markdown(app_handle: AppHandle, file_path: &str) -> Result<RenderedDocument, String> {
...
#[tauri::command]
pub async fn export_markdown(app_handle: AppHandle, file_path: &str) -> Result<String, String> {
...
/// Save (copy) an existing PDF (already rendered/exported) to a user-selected
/// destination. If the source is a markdown path, we compile first to ensure
/// it's up to date, then copy.
#[tauri::command]
pub async fn save_pdf_as(app_handle: AppHandle, file_path: &str, destination: &str) -> Result<String, String> {
...
#[tauri::command]
pub async fn render_typst(app_handle: AppHandle, content: &str, format: &str, current_file: Option<&str>) -> Result<RenderedDocument, String> {
